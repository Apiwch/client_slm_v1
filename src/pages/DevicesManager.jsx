...
function DevicesManager() {
    ...
    useEffect(() => {
        ...
        axios.get('https://18.143.173.11/api/home', { headers: { Authorization: token } })
            .then((response) => {
    ...
    useEffect(() => {
        axios.get(`https://18.143.173.11/api/Devices/${username}`)
            .then(response => {
    ...
    const HandleAddDevice = (e) => {
        ...
        axios.post('https://18.143.173.11/api/newDevice', { newDeviceName, newDeviceSerial, lat, lon, username })
            .then((response) => {
    ...
    const HandleDelDevice = (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                axios.delete(`https://18.143.173.11/api/deleteDevice/${id}`)
                    .then((response) => {
                        setAddMessage(response.data);
                        Swal.fire({
                            title: "Deleted!",
    ...
    const HandleUpdateDevice = (e) => {
        e.preventDefault()
        axios.put(`https://18.143.173.11/api/updateDevice/${id}`, { newDeviceName, newDeviceSerial, lat, lon, username })
            .then((response) => {
                setAddMessage(response.data);
                setShow(false);
                Swal.fire({
