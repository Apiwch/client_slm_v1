...
function Home() {
  ...
  const fetchLogin = async () =>{
    try{
      const response = await axios.get('https://18.143.173.11/api/home', { headers: { Authorization: token } })
  ...
  const fetchDevice = async () =>{
    try{
      setLoading(true);
      const response = await axios.get(`https://18.143.173.11/api/Devices/${username}`);
  ...
  useEffect(() => {
    const socket = new WebSocket('wss://18.143.173.11');
  ...
  function checkStatus(device) {
    ...
    if (new Date(Date.parse(device.timestamp)).getTime() <= threeSecondsAgo.getTime()) {
      return 'Offline';
    } else {
      return 'Online';
    }
